...
*   - Mejorado en método asignar, ahora se le puede pasar un array() con los pares variable => valor
...
class NokTemplate {
    ...
    **/
    function cargar($clave, $nombreArchivo) {
    //
        if(!empty($clave) AND !empty($nombreArchivo) AND $this->_usarCache != 2) {
    ...
    **/
    function cargarVirtual($clave, $contenido) {
    //
        if(!empty($clave) AND !empty($contenido) AND $this->_usarCache != 2) {
            $this->_archivosArray[$clave] = $contenido;
        }
    }
    ...
    */
    ...
    //
		if (is_array($variable)) {
			$this->_variablesArray = array_merge($this->_variablesArray, $variable);
		}
		else {
			if (!empty($variable)) {
            	$this->_variablesArray[$variable] = $valor;
        	}
        	else {
            	$this->error('Identificador nulo. Debe especificar alguno');
        	}
		}
    }
    ...
    * @param boolean $analizar Especifica si se analizara en busqueda de variables o no el template.
    * @access public
    */
    function expandir($variable, $clave, $analizar = True) {
    //
    ...
    */
    ...
    //
        if(!empty($variable) AND $this->_usarCache > 0) {
            $this->_variableDinamica = True;
            $this->_variablesArray[$variable] = $this->_tagsVarDinamicaArray['abrir'] ." ". $variable ." ". $this->_tagsVarDinamicaArray['close'] . $valor . $this->_tagsVarDinamicaArray['cerrar'] ." ". $variable ." ". $this->_tagsVarDinamicaArray['close'];
        }
        else {
    ...
    */
	...
	/*
    * NokTemplate::autoExpandir()
    * @access public
    * @param .
    */
    function autoExpandir($variable, $clave, $valores) {
	//
		$clave = ($clave{0}=='+')?substr($clave, 1):$clave;
		foreach($valores as $var => $valor) {
			if (is_array($valor)) {
				$this->asignar($valor);
			}
			else{
				$this->asignar($var, $valor);
			}
			$this->expandir($variable, '+'.$clave);
		}
	}
}
?>
